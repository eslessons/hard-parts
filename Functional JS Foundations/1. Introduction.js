...
paradigm for structuring
our complex code

Let’s suppose we have a quiz game we’re building

Every line of code either saves data to memory (e.g. a user’s
score) or uses that data (e.g. increase that user’s score)
...
consider whether it could affect other lines (and when I miss
...
Tiny functions: Save every single line (or few lines) as its
own function

No consequences except on that line 
Each function’s only n‘consequence’ is to have its result given to specifically the next line of code (‘function call’) and not to any
other lines

Recombine/compose 
Build up our application by using these small blocks of 
self-contained code combining them up line-by-line by 
referring to their human-readable name
...
Combining our functions is going to require a ton of interesting
techniques to:
...
reusable
	— They’re small enough that they’re tasks like
	incrementing a number, looping through an array
...
It’s especially important when you’re reusing/
recombining lots of these little single-step functions in
lots of totally different scenarios - they better be self
contained!

(4) Closure, (5) Function Decoration (6) Partial application
& Currying

We will need ways to:

	— Adjust the functions in case they don’t quite fit
	together as initially saved
